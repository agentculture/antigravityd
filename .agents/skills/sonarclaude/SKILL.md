...
bash .agents/skills/sonarclaude/scripts/sonar.sh status

# List issues (bugs, vulnerabilities, code smells)
bash .agents/skills/sonarclaude/scripts/sonar.sh issues

# Filter issues by severity and type
bash .agents/skills/sonarclaude/scripts/sonar.sh issues --severity CRITICAL --type BUG

# Key metrics (coverage, bugs, code smells, duplication, LOC)
bash .agents/skills/sonarclaude/scripts/sonar.sh metrics

# Security hotspots
bash .agents/skills/sonarclaude/scripts/sonar.sh hotspots

# Accept (won't-fix) an OPEN issue with a rationale comment
bash .agents/skills/sonarclaude/scripts/sonar.sh accept \
  --issue AZ3Ep83i4ywi8V_l99p0 \
  --comment "Pushback: rule premise doesn't fit our test fixture pattern."

# Different project
bash .agents/skills/sonarclaude/scripts/sonar.sh status --project OtherOrg_OtherProject

# Raw JSON output
bash .agents/skills/sonarclaude/scripts/sonar.sh issues --raw

# Limit results
bash .agents/skills/sonarclaude/scripts/sonar.sh issues --limit 10
```
